...
class PurchaseOrderController extends Controller
{
    public function purchase_order_list()
    {
        ...
        $purchaseItems = Purchaseitems_order::whereIn('purchase_id', $purchaseIds)->get();
        ...
        return view('admin.purchase.purchase_order_list', compact('purchase', 'suppliers', 'logo', 'account', 'purchaseItems'));
    }
    ...
    public function new_purchase_order()
    {
        ...
        return view('admin.purchase.add_purchase_order', compact('supplier', 'account', 'ware', 'tax', 'store', 'unit', 'logo', 'category', 'brands', 'country'));
    }

    public function add_purchase_order(Request $request)
    {
        $lastPurchase = Purchase_order::orderBy('id', 'desc')->first();
        ...
        $maxPurchaseId = Purchase_order::max('count_id');
        ...
        if (
            ...
            for ($i = 0; $i < count($item_ids); $i++) {
                $purchaseItems = Purchaseitems_order::create([
                    'item_id' => $item_ids[$i],
        ...
        $sales_itemdata = Purchaseitems_order::where('purchase_id', $purchase->id)->get();
        $items = Item::whereIn('id', $sales_itemdata->pluck('item_id'))->get(); // Use whereIn to get all matching items
        $logo = Coresetting::all();

        return redirect()->route('invoice_purchase.order', ['purchase' => $purchase->id])
            ->with(compact('sales_itemdata', 'logo', 'items', 'purchaseItems'));

    }


    public function purchase_add_order(Request $request, $id)
    {
        ...
        if ($purchase) {
            ...
            $purchaseItems = Purchaseitems_order::where('purchase_id', $purchase->id)
        ...
        $account = Account::all();
        $logo = Coresetting::all();
        return view('admin.purchase.add_to_purchase', compact('taxes', 'purchaseItems', 'units', 'logo', 'account', 'purchase', 'supplier', 'suppliers', 'country', 'ware', 'tax', 'category', 'brands', 'unit', 'store', 'stores', 'items'));
    }
public function add_to_purchase_order(Request $request)
    {
        ...
        if (
            ...
            for ($i = 0; $i < count($item_ids); $i++) {
                $itemTaxId = is_array($tax_id) ? $tax_id[$i] : $tax_id;
