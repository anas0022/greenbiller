...
{
    public function Purchase_order_sale()
    {
        $sales = purchase_order_sale::all();
        ...
        return view('admin.salepos.purchase_order_salelist', compact('sales', 'suppliers', 'user', 'logo', 'account'));

    }

    public function add_Purchase_order_sale(Request $request)
    {
        $ware = Warehouse::all();
   ...
   public function create_purchase_order_sale(Request $request){
    DB::beginTransaction();
    ...
    try {
        ...
        $totalbilledamt = purchase_order_sale::where('customer_id', $customer_id)->sum('grand_total');

        $totalpayedamt = purchase_order_sale::where('customer_id', $customer_id)->sum('paid_amount');
        ...
        $lastSaleEntry = purchase_order_sale::where('store_id', $store_id)
        ...
        $sale = purchase_order_sale::create([
        ...
        if (
            ...
            for ($i = 0; $i < count($item_ids); $i++) {
                    ...
                    $normalsale = purchase_order_sales_items::create([
        ...
        $logo = Coresetting::all();
        $sales_itemdata = purchase_order_sales_items::where('sales_id', $sale->id)->get();
        ...
        return redirect()->route('invoice.purchase.sale', ['id' => $sale->id])
   ...
   public function create_purchase_order_sale_post(Request $request , $id){
    $logo = Coresetting::all();
    $sale = purchase_order_sale::find($id); 


    $sales_itemdata = purchase_order_sales_items::where('sales_id', $id)->where('status', '0')->get();
    ...
    return view('admin.salepos.purchase_addto_sale', compact(
        'cu_store',
        'sale',
        'qty',
        'other_charge',
