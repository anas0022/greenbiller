...
namespace App\Http\Controllers\store\closing;
...
class ClosingController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            // Fetch all closing records only for the authenticated user's store_id
            $closing = Closing::where('store_id', Auth::user()->store_id)
            ->select('id', 'date', 'invoice_count', 'total_sale', 'total_expense', 'closing_amount', 'opening_balance', 'prefix', 'closing_code', 'purchase_count', 'total_purchase', 'store_id')
        ...
        return view('store.closing.closinglist', [
            'logo' => $logo
        ]);
    }
    ...
    {       
        ...
        return view('store.closing.dailyclosing', [
            'logo' => $logo,
            'sale' => $initial_sales,
            'sale_amount' => [],
            'expense' => [],
