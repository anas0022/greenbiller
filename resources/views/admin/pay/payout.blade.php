...
@section('content')
                ...
                <div class="col-md-12">
                    <div class="card ">
                        <div class="card-header  text-white bg-primary">
                            <h4 class="mb-0"><i class="fas fa-receipt me-2"></i>Payment Out</h4>
                            <button class="btn bg-primary "style="color:white;">Transactions</button>
                        </div>
                        <div class="card-body ">
                            <form action="{{ route('payout.post') }}" method="post">
                                @csrf
                                <div class="row g-4 mb-4">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label fw-bold text-primary">
                                                <i class="fas fa-user me-1 text-primary"></i>Select Supplier
                                            </label>
                                            <select id="supplier_select" name="supplier_id" class="form-select selectpicker"
                                                data-live-search="true" onchange="suppliercheck()">
                                    ...
                                    function suppliercheck() {
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(response) {
                                                var supplierdata = JSON.stringify(response);
                                                response.forEach(function(supplierdata) {
                                                    document.getElementById('prevdue').innerHTML = 'Previous Due : ' + supplierdata
                                                        .previous_due;
                                ...
                                <div class="card mb-8 border-info">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="col-md-6">
                                        <label for="" class="form-label">Type<span class="required">*</span></label>
                                            <select name="type" id="" class="form-control form-control-sm col-sm-6" style="cursor:pointer;">
                                    ...
                                    <div class="col-md-6">
                                    <label for="" class="form-label">Account <span class="required">*</span></label>
                                            <select name="account_id" id="" class="form-control form-control-sm col-sm-6">
                                                
                                        <option value="" >-select-</option>
                                        @foreach ($account as $name)
                                            <option value="{{ $name->id }}">{{ $name->account_name }}</option>
                                        @endforeach
                                    </select>
                                    </div>
                                    </div>
 ...
 <script>
    ...
    $(document).on('change', '#supplier_select', function() {
        ...
        $.ajax({
            url: '{{ route('get.bill.purchase') }}',
            type: 'GET',
            data: { supplier_id: supplier },
            success: function(response) {
                $('#salepay').empty();
                if (response.success && response.purchases && response.purchases.length > 0) {
                    $.each(response.purchases, function(key, purchase) {
                        // Calculate current balance
                        var currentBalance = Math.round(purchase.grand_total) - Math.round(purchase.paid_amount);
                        ...
                        $('#salepay').append(
                            '<div class="sale-item">' +
                                ...
                                '<div class="sale-details">' +
                                    '<div class="bill-no">Bill No: ' + purchase.prefix + '/' + purchase.purchase_code + '</div>' +
                                    '<div class="total-amount">Total Amount: ₹' + Math.round(purchase.grand_total) + '</div>' +
                                    '<input type="hidden" name="purchase_id[]" value="' + purchase.id + '" class="purchase-id">' +
                                '</div>' +
                                '<div class="sale-details">' +
                    ...
                    $('#salepay').append('<div class="no-sales">No Purchases found from this supplier.</div>');
                }
            }
        });
    });
    ...
    selectedItems.push({
     ...
     currentBalance: item.find('#current').text().trim(),
      linkAmount: item.find('.link-amount').val().trim(),
        saleid: item.find('.purchase-id').val().trim() 
    });
});
                        ...
                        <div class="d-flex align-items-start mb-2">
                            ...
                            <div class="bill-number">
                                ...
                                <h6 class="mb-0 text-primary">${item.billNo}</h6>
                                 <input type="hidden" name="bill_no[]" value="${item.billNo}">
                            </div>
                        </div>
                    </div>
                    <span class="badge bg-primary ms-2">₹${item.linkAmount}</span>
                    <input type="hidden" name="totalLinkAmount[]" value="${item.linkAmount}">
                    <input type="hidden" class="sale-id" value="${item.saleid}" name="id_sale[]">
                </div>
...
@media (max-width: 768px) {
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#supplier_select').on('focus', function(e) {
            ...
            $('#store_select').on('change', function() {
                ...
                if (storeId) {
                    $.ajax({
                        url: '{{ route('get.suppliers.by.store') }}',
                        type: 'GET',
                        data: {
                            store_id: storeId
                        },
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                swal({
                                    ...
                                    text: "There are no suppliers associated with this store.",
                                    icon: "warning",
                                    button: "OK",
                                });
                                ...
                                $.each(data, function(key, value) {
                                    $('#supplier_select').append('<option value="' +
                                        value.id + '">' + value.name +
                                        '</option>');
                                });
                                // Refresh selectpicker after updating options
                                $('#supplier_select').selectpicker('refresh');
                            }
                        },
                        error: function() {
                            swal({
                    ...
                    $('#supplier_select').empty();
                    $('#supplier_select').append('<option value="">Select Supplier</option>');
                    $('#supplier_select').selectpicker('refresh');
                }
            });
        });
